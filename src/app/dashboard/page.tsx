...
function Sidebar({
  ...
  setSelectedMonitor: (id: string | null) => void;
    ...
    if (!loading && activeMonitorId && monitors.length > 0) {
      ...
      const timer = setTimeout(() => {
        const selectedButton = document.querySelector(
          `[data-monitor-id="${activeMonitorId}"]`
        ) as HTMLElement;
        if (selectedButton) {
  ...
  const filteredItems = monitors.filter((item) => {
    ...
    if (item.config) {
      ...
      if (
        config.url &&
        String(config.url).toLowerCase().includes(searchQuery.toLowerCase())
      ) {
        return true;
      }
      ...
      if (
        config.hostname &&
        config.port &&
        `${String(config.hostname)}:${String(config.port)}`
          .toLowerCase()
          .includes(searchQuery.toLowerCase())
      ) {
        return true;
                  ...
                  >
                    <i
                      className={`${getIconForType(
                        item.type
                      )} w-6 h-6 mr-3.5 flex items-center justify-center flex-shrink-0`}
                    ></i>
                    <span className="flex-grow truncate mr-3.5 font-medium">
                      {item.name}
                    </span>
                    <div
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead className="dark:bg-dark-nav bg-light-nav">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-medium text-foreground/80">
                名称
              </th>
              <th className="px-6 py-4 text-left text-sm font-medium text-foreground/80">
                状态
              ...
              >
                <td className="px-6 py-4">
                  <div className="flex items-center">
                    <div
                      className={`w-2.5 h-2.5 rounded-full ${getStatusDotClass(
                        item.status
                      )} mr-2 flex-shrink-0`}
                    ></div>
                    <Link
...
const calculateUptime = (statusHistory: HistoryRecord[] | undefined) => {
  ...
  const successChecks = statusHistory.filter(
...
const calculateAvailability = (statusHistory: HistoryRecord[] | undefined) => {
  ...
  const successChecks = recentHistory.filter(
    (record) => record.status === 1
  ).length;
...
export default function DashboardPage() {
  ...
  const [monitorData, setMonitorData] = useState<MonitorDetailData | null>(
    null
  );
          ...
          setMonitorData({
            ...
            message:
              data.lastStatus === 1
                ...
                : "未知状态",
            responseTime:
              data.statusHistory && data.statusHistory[0]?.ping
                ? `${data.statusHistory[0].ping}ms`
                : "N/A",
