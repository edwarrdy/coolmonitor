...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (type) {
      ...
      case "企业微信推送":
        return await testWorkWechatNotification(
          name,
          config as WorkWechatConfig
        );
      default:
        return NextResponse.json(
...
async function testEmailNotification(name: string, config: EmailConfig) {
  ...
  if (!email || !smtpServer || !smtpPort) {
    return NextResponse.json(
      {
        success: false,
        error: "邮件配置不完整，请检查收件人地址、SMTP服务器和端口",
      },
      { status: 400 }
    );
  }
    ...
    const info = await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #6366F1; border-radius: 10px;">
          <h2 style="color: #6366F1;">🔔 东2信息化监控通知测试</h2>
          <p>您好，这是来自 <strong>东2信息化监控</strong> 系统的测试通知邮件。</p>
...
>>>>>>> a58d9eb9d09ee4b48941c365c50a575c511439e4
    ...
    if (response.status >= 200 && response.status < 300) {
      return NextResponse.json({
        success: true,
        message: `测试Webhook请求已成功发送，响应状态码: ${response.status}`,
      });
    } else {
      return NextResponse.json(
        {
          success: false,
          error: `Webhook请求失败，响应状态码: ${response.status}`,
        },
        { status: 400 }
      );
    }
...
async function testDingTalkNotification(name: string, config: DingTalkConfig) {
  ...
  try {
    ...
    console.log(
      `钉钉推送测试响应: 状态码=${response.status}, 数据=${JSON.stringify(
        response.data
      )}`
    );
    ...
    if (response.data && response.data.errcode !== undefined) {
      if (response.data.errcode === 0) {
        ...
        return NextResponse.json(
          {
            success: false,
            error: `钉钉API返回错误: ${response.data.errmsg || "未知错误"}`,
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        {
          success: false,
          error: `钉钉推送请求失败，响应状态码: ${response.status}`,
        },
        { status: 400 }
      );
    }
...
async function testWechatNotification(name: string, config: WechatConfig) {
  ...
  try {
    // 准备测试数据
    const title = "东2信息化监控 - 测试通知";
    const content = `## 东2信息化监控通知测试\n\n这是来自东2信息化监控系统的测试通知。\n\n- **通知渠道**: ${name}\n- **测试时间**: ${formatDateTime()}\n\n如果您收到此通知，表示您的微信推送设置已配置成功！`;
    ...
    console.log(
      `微信推送测试响应: 状态码=${response.status}, 数据=${JSON.stringify(
        response.data
      )}`
    );
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `微信推送请求失败，响应状态码: ${response.status}`,
        },
        { status: 400 }
      );
    }
...
// 测试企业微信推送通知
async function testWorkWechatNotification(
  name: string,
  ...
  try {
    ...
    console.log(
      `企业微信推送测试响应: 状态码=${response.status}, 数据=${JSON.stringify(
        response.data
      )}`
    );
    ...
    if (response.data && response.data.errcode !== undefined) {
      if (response.data.errcode === 0) {
        return NextResponse.json({
          success: true,
          message: "测试企业微信推送已成功发送",
        });
      } else {
        return NextResponse.json(
          {
            success: false,
            error: `企业微信API返回错误: ${response.data.errmsg || "未知错误"}`,
          },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        {
          success: false,
          error: `企业微信推送请求失败，响应状态码: ${response.status}`,
        },
        { status: 400 }
      );
    }
