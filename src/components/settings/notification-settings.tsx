...
export function NotificationSettings({
  ...
  const [currentEditingNotification, setCurrentEditingNotification] =
  ...
  const handleAddNotification = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const defaultForNewMonitors = document.getElementById(
        "defaultForNewMonitors"
      ) as HTMLInputElement;
  ...
  const handleEditNotification = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/settings/notifications/${currentEditingNotification.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const getFormConfig = (type: NotificationType): Record<string, unknown> => {
    ...
    switch (type) {
      case "邮件": {
        ...
        const smtpServer = form.querySelector(
          'input[name="smtpServer"]'
        ) as HTMLInputElement;
        const smtpPort = form.querySelector(
          'input[name="smtpPort"]'
        ) as HTMLInputElement;
      ...
      case "微信推送": {
        const pushUrl = form.querySelector(
          'input[name="pushUrl"]'
        ) as HTMLInputElement;
        return {
          pushUrl: pushUrl?.value || "",
          titleTemplate: "东2信息化监控 - {monitorName} 状态变更",
          contentTemplate:
            "## 监控状态变更通知\n\n- **监控名称**: {monitorName}\n- **监控类型**: {monitorType}\n- **当前状态**: {status}\n- **变更时间**: {time}\n\n{message}",
        };
      }
      case "钉钉推送": {
        const webhookUrl = form.querySelector(
          'input[name="dingtalkWebhookUrl"]'
        ) as HTMLInputElement;
        const secret = form.querySelector(
          'input[name="dingtalkSecret"]'
        ) as HTMLInputElement;
      ...
      case "企业微信推送": {
        const webhookUrl = form.querySelector(
          'input[name="workWechatWebhookUrl"]'
        ) as HTMLInputElement;
  ...
  const resetForm = () => {
    ...
    const smtpServerInput = form.querySelector(
      'input[name="smtpServer"]'
    ) as HTMLInputElement;
    if (smtpServerInput) smtpServerInput.value = "";

    const smtpPortInput = form.querySelector(
      'input[name="smtpPort"]'
    ) as HTMLInputElement;
    ...
    // 重置Webhook表单
    const webhookUrlInput = form.querySelector(
      'input[name="webhookUrl"]'
    ) as HTMLInputElement;
    if (webhookUrlInput) webhookUrlInput.value = "";

    // 重置微信推送表单
    const pushUrlInput = form.querySelector(
      'input[name="pushUrl"]'
    ) as HTMLInputElement;
    if (pushUrlInput) pushUrlInput.value = "";

    // 重置钉钉推送表单
    const dingtalkWebhookUrlInput = form.querySelector(
      'input[name="dingtalkWebhookUrl"]'
    ) as HTMLInputElement;
    if (dingtalkWebhookUrlInput) dingtalkWebhookUrlInput.value = "";

    const dingtalkSecretInput = form.querySelector(
      'input[name="dingtalkSecret"]'
    ) as HTMLInputElement;
    if (dingtalkSecretInput) dingtalkSecretInput.value = "";

    // 重置企业微信推送表单
    const workWechatWebhookUrlInput = form.querySelector(
      'input[name="workWechatWebhookUrl"]'
    ) as HTMLInputElement;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold dark:text-foreground text-light-text-primary">
          通知设置
        </h3>
        <button
          onClick={(e) => {
            ...
            >
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center">
                  <div
                    className={`w-10 h-10 rounded-full ${
                      notification.enabled ? "bg-primary/20" : "bg-gray-500/20"
                    } flex items-center justify-center mr-3`}
                  >
                    <i
                      className={`fas ${
                        notification.type === "邮件"
                          ? "fa-envelope"
                          : notification.type === "Webhook"
                          ? "fa-link"
                          : notification.type === "微信推送"
                          ? "fa-weixin"
                          : notification.type === "钉钉推送"
                          ? "fa-bell"
                          : notification.type === "企业微信推送"
                          ? "fa-building"
                          : "fa-paper-plane"
                      } ${
                        notification.enabled ? "text-primary" : "text-gray-500"
                      }`}
                    ></i>
                  </div>
                  <div>
                    <h4 className="text-md font-medium dark:text-foreground text-light-text-primary">
                      {notification.name}
                    </h4>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="relative inline-block w-12 h-6 transition duration-200 ease-in-out">
                    ...
                    >
                      <span
                        className={`block h-6 w-6 rounded-full bg-white shadow-md transform transition-transform duration-200 ease-in-out toggle-dot ${
              ...
              {notification.enabled && notification.type === "邮件" && (
                <div className="mt-3 pl-11 text-sm dark:text-foreground/80 text-light-text-secondary bg-primary/5 p-2 rounded-lg">
                  <p>
                    SMTP: {String(notification.config.smtpServer)}:
                    {String(notification.config.smtpPort)}
                  </p>
                  <p>
              ...
              {notification.enabled && notification.type === "Webhook" && (
                <div className="mt-3 pl-11 text-sm dark:text-foreground/80 text-light-text-secondary bg-primary/5 p-2 rounded-lg">
                  <p>
                    Webhook URL: {String(notification.config.url) || "未设置"}
                  </p>
                  {notification.defaultForNewMonitors && (
                    <p className="mt-1 text-xs text-yellow-400 flex items-center">
                      <i className="fas fa-star mr-1"></i>
                ...
                <div className="mt-3 pl-11 text-sm dark:text-foreground/80 text-light-text-secondary bg-primary/5 p-2 rounded-lg">
                  <p>
                    ...
                    <span className="font-mono text-xs">
                      {String(notification.config.pushUrl) || "未设置"}
                    </span>
                  </p>
              ...
              {notification.enabled && notification.type === "钉钉推送" && (
                <div className="mt-3 pl-11 text-sm dark:text-foreground/80 text-light-text-secondary bg-primary/5 p-2 rounded-lg">
                  <p>
                    Webhook地址:{" "}
                    <span className="font-mono text-xs">
                      {String(notification.config.webhookUrl) || "未设置"}
                    </span>
                  </p>
      ...
      {(isAddModalOpen || currentEditingNotification) && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4">
          <div className="bg-dark-card dark:bg-dark-card bg-light-card w-full max-w-4xl max-h-[90vh] rounded-xl shadow-2xl border border-primary/25 animate-fadeIn flex flex-col">
            ...
            >
              <div>
                <label className="text-xs font-medium dark:text-foreground text-light-text-primary">
                  通知类型
                </label>
              ...
              <div className="flex items-center">
                <input
                  ...
                  defaultChecked={
                    currentEditingNotification?.defaultForNewMonitors || true
                  }
                />
                <label
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={
                        (currentEditingNotification?.config?.email as string) ||
                        ""
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={
                        (currentEditingNotification?.config
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      defaultValue={
                        (currentEditingNotification?.config?.url as string) ||
                        ""
                      }
                    />
                    <p className="mt-1 text-xs dark:text-foreground text-light-text-secondary">
                  ...
                  <div className="mt-3 p-4 bg-primary/5 rounded-lg border border-primary/10">
                    <h3 className="text-sm font-medium mb-3 dark:text-foreground text-light-text-primary">
                      Webhook通知数据格式
                    </h3>
  ...
  "failure_info": {              // 仅在状态为异常时存在
    ...
    "duration_minutes": 15       // 失败持续时间(分钟)
                    ...
                    <p className="text-xs mt-3 dark:text-foreground/60 text-light-text-secondary">
                      系统通过POST请求发送JSON格式数据，Content-Type为application/json
                    </p>
                  </div>
                </div>
              )}
              ...
              {selectedType === "微信推送" && (
                <div>
                  ...
                  <input
                    ...
                    defaultValue={
                      (currentEditingNotification?.config?.pushUrl as string) ||
                      ""
                    }
                  />
                  <p className="mt-1 text-xs dark:text-foreground text-light-text-secondary">
                ...
                <div className="space-y-4">
                  ...
                  <div className="mt-3 p-4 bg-primary/5 rounded-lg border border-primary/10">
                    ...
                    <ol className="text-xs dark:text-foreground/70 text-light-text-secondary list-decimal pl-5 space-y-2">
                      ...
                      <li>
                        安全设置选择&ldquo;加签&rdquo;方式（推荐）或&ldquo;自定义关键词&rdquo;
                      </li>
                      <li>
                        如选择加签方式，请将生成的密钥填入上方&ldquo;加签密钥&rdquo;字段
                      </li>
                      <li>
                        如选择关键词方式，请添加&ldquo;东2信息化监控&rdquo;作为关键词
                      </li>
              ...
              <button
                onClick={(e) => {
                  ...
                  const config = getFormConfig(
                    selectedType as NotificationType
                  );
