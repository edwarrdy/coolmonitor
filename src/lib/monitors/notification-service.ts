...
export async function sendStatusChangeNotifications(
  ...
  try {
    ...
    // 如果没有启用的通知配置，则不发送
    if (
      !monitor.notificationBindings ||
      monitor.notificationBindings.length === 0
    ) {
      return;
    }
    ...
    const isNewMonitor =
      !monitor.statusHistory || monitor.statusHistory.length <= 1;
    ...
    if (
      ...
      monitor.statusHistory.length > 1
    ) {
      const prevStatusFromHistory = monitor.statusHistory[1]?.status;
      if (
        prevStatusFromHistory !== undefined &&
        prevStatusFromHistory !== null
      ) {
    ...
    if (status === 0) {
      ...
      if (monitor.resendInterval > 0) {
        ...
        if (lastNotification && lastNotification.status === 0) {
          ...
          const failuresSinceLastNotification =
            await prisma.monitorStatus.count({
              where: {
                monitorId,
                status: 0,
                timestamp: {
      ...
      const duration = firstContinuousFailure
        ? Math.floor(
            (now - firstContinuousFailure.timestamp.getTime()) / 1000 / 60
          )
        : 0;
      ...
      const aggregatedData = {
        ...
        firstFailureTime: firstContinuousFailure
          ? formatDateTime(firstContinuousFailure.timestamp)
          : "未知",
        lastFailureTime: formatDateTime(),
        failureDuration: duration,
        message: `连续失败 ${totalFailures} 次，首次失败于 ${
          firstContinuousFailure
            ? formatDateTime(firstContinuousFailure.timestamp)
            : "未知"
        }，持续 ${duration} 分钟\n${notificationData.message}`,
      };
      ...
      for (const binding of monitor.notificationBindings) {
        ...
        try {
          ...
          console.error(
            `向 ${channel.name}(${channel.type}) 发送失败通知失败:`,
            error
          );
        }
      }
      ...
      const recoverDuration =
        lastNotification && lastNotification.status === 0
      ...
      const recoveryData = {
        ...notificationData,
        message: `监控已恢复正常。${
          recoverDuration > 0 ? `故障持续了约 ${recoverDuration} 分钟。` : ""
        }\n${notificationData.message}`,
      };
      ...
      for (const binding of monitor.notificationBindings) {
        ...
        try {
          ...
          console.error(
            `向 ${channel.name}(${channel.type}) 发送恢复通知失败:`,
            error
          );
        }
      }
      ...
      for (const binding of monitor.notificationBindings) {
        ...
        try {
          ...
          console.error(
            `向 ${channel.name}(${channel.type}) 发送通知失败:`,
            error
          );
        }
      }
...
async function sendEmailNotification(
  ...
  // 构建邮件内容
  const subject = `东2信息化监控 - ${data.monitorName} 状态${data.statusText}`;
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #6366F1; border-radius: 10px;">
      ...
      <div style="background-color: ${
        data.status === "UP" ? "#10B981" : "#EF4444"
      }1a; padding: 15px; border-radius: 8px; margin: 15px 0;">
...
async function sendWebhookNotification(
  ...
  console.log(
    `发送Webhook通知: URL=${url}, 监控项=${data.monitorName}, 状态=${data.statusText}`
  );
  ...
  try {
    ...
    console.log(
      `Webhook通知发送成功: 状态码=${response.status}, 监控项=${data.monitorName}`
    );
    return response;
  } catch (error) {
    if (axios.isAxiosError(error)) {
      if (error.response) {
        console.error(
          `Webhook通知发送失败，服务器响应: 状态码=${
            error.response.status
          }, 数据=${JSON.stringify(error.response.data)}, 监控项=${
            data.monitorName
          }`
        );
      } else if (error.request) {
        console.error(
          `Webhook通知发送失败，无响应: ${error.message}, 监控项=${data.monitorName}`
        );
      } else {
        console.error(
          `Webhook通知发送失败，请求配置错误: ${error.message}, 监控项=${data.monitorName}`
        );
      }
    } else {
      console.error(
        `Webhook通知发送失败，未知错误: ${error}, 监控项=${data.monitorName}`
      );
    }
    throw error;
  }
...
async function sendWechatNotification(
  ...
  // 替换模板中的变量
  let title = titleTemplate || "东2信息化监控 - {monitorName} 状态{statusText}";
  let content =
    contentTemplate ||
    "## 监控状态变更通知\n\n" +
      "- **监控名称**: {monitorName}\n" +
...
async function sendDingTalkNotification(
  ...
  // 构建消息内容
  let content = "";
  const title = `东2信息化监控 - ${data.monitorName} 状态${data.statusText}`;
  ...
  content =
    ...
    `- **当前状态**: <font color="${
      data.statusCode === 1 ? "#10B981" : "#EF4444"
  ...
  console.log(
    `发送钉钉通知: URL=${webhookUrl}, 监控项=${data.monitorName}, 状态=${data.statusText}`
  );
  ...
  try {
    ...
    console.log(
      `钉钉通知发送成功: 状态码=${response.status}, 监控项=${data.monitorName}`
    );
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        console.error(
          `钉钉通知发送失败，无响应: ${error.message}, 监控项=${data.monitorName}`
        );
      } else {
        console.error(
          `钉钉通知发送失败，请求配置错误: ${error.message}, 监控项=${data.monitorName}`
        );
      }
    } else {
      console.error(
        `钉钉通知发送失败，未知错误: ${error}, 监控项=${data.monitorName}`
      );
    }
    throw error;
  }
...
async function sendWorkWechatNotification(
  ...
  const content = {
    ...
    markdown: {
      content:
        ...
        `**监控类型**: ${data.monitorType}\n` +
        `**当前状态**: <font color="${
          data.statusCode === 1 ? "info" : "warning"
        }">${data.statusText}</font>\n` +
        `**变更时间**: ${data.time}\n` +
        (data.failureCount
          ? `**连续失败次数**: ${data.failureCount} 次\n` +
  ...
  console.log(
    `发送企业微信通知: URL=${webhookUrl}, 监控项=${data.monitorName}, 状态=${data.statusText}`
  );
  ...
  try {
    ...
    console.log(
      `企业微信通知发送成功: 状态码=${response.status}, 监控项=${data.monitorName}`
    );

    // 检查企业微信API返回结果
    if (response.data && response.data.errcode !== 0) {
      throw new Error(
        `企业微信API返回错误: ${response.data.errmsg || "未知错误"}`
      );
    }
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        ...
        console.error(
          `企业微信通知发送失败，无响应: ${error.message}, 监控项=${data.monitorName}`
        );
      } else {
        console.error(
          `企业微信通知发送失败，请求配置错误: ${error.message}, 监控项=${data.monitorName}`
        );
      }
    } else {
      console.error(
        `企业微信通知发送失败，未知错误: ${error}, 监控项=${data.monitorName}`
      );
    }
    throw error;
  }
